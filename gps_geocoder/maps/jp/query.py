...
def reverse_geocode(lat: float, lng: float) -> dict:
    ...
    conn = _get_conn()
    try:
        city = _find_admin(conn, lat, lng, 4) or _find_admin(conn, lat, lng, 5)  # region or prefecture
        district = _find_admin(conn, lat, lng, 7)  # city/ward
        village = _find_admin(conn, lat, lng, 8)    # town/village
